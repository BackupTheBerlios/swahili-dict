...
var arrFontname=['ç´°æé«','é»é«','æ¥·é«','é¸æ¸','å¹¼å','å¾®è»éé»','Arial','Arial Narrow','Arial Black','Comic Sans MS','Courier New','System','Times New Roman','Tahoma','Verdana'];
var arrFontsize=[{n:'xx-small',wkn:'x-small',s:'8pt',t:'æ¥µå°'},{n:'x-small',wkn:'small',s:'10pt',t:'ç¹å°'},{n:'small',wkn:'medium',s:'12pt',t:'å°'},{n:'medium',wkn:'large',s:'14pt',t:'ä¸­'},{n:'large',wkn:'x-large',s:'18pt',t:'å¤§'},{n:'x-large',wkn:'xx-large',s:'24pt',t:'ç¹å¤§'},{n:'xx-large',wkn:'-webkit-xxx-large',s:'36pt',t:'æ¥µå¤§'}];
var menuAlign=[{s:'å·¦å°é½',v:'justifyleft',t:'å·¦å°é½'},{s:'å±ä¸­',v:'justifycenter',t:'å±ä¸­'},{s:'å³å°é½',v:'justifyright',t:'å³å°é½'},{s:'å©ç«¯å°é½',v:'justifyfull',t:'å©ç«¯å°é½'}],menuList=[{s:'æ¸å­åè¡¨',v:'insertOrderedList',t:'æ¸å­åè¡¨'},{s:'ç¬¦èåè¡¨',v:'insertUnorderedList',t:'ç¬¦èåè¡¨'}];
var htmlPastetext='<div>ä½¿ç¨éµç¤å¿«æ·éµ(Ctrl+V)æå§å®¹ç²è²¼å°æ¹æ¡è£¡ï¼æ ç¢ºå®</div><div><textarea id="xhEdtPastetextValue" wrap="soft" spellcheck="false" style="width:300px;height:100px;" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="ç¢ºå®" /></div>';
var htmlLink='<div>éæ¥å°å: <input type="text" id="xhEdtLinkHref" value="http://" class="text" /></div><div>æéæ¹å¼: <select id="xhEdtLinkTarget"><option selected="selected" value="">é»èª</option><option value="_blank">æ°çªå£</option><option value="_self">ç¶åçªå£</option><option value="_parent">ç¶çªå£</option></select></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="ç¢ºå®" /></div>';
var htmlImg='<div>åçå°åï¼<input type="text" id="xhEdtImgSrc" value="http://" class="text" /></div><div>æ¿æææ¬ï¼<input type="text" id="xhEdtImgAlt" /></div><div>å°é½æ¹å¼ï¼<select id="xhEdtImgAlign"><option selected="selected" value="">é»èª</option><option value="left">å·¦å°é½</option><option value="right">å³å°é½</option><option value="top">é ç«¯</option><option value="middle">å±ä¸­</option><option value="baseline">åºç·</option><option value="bottom">åºé</option></select></div><div>å¯¬åº¦é«åº¦ï¼<input type="text" id="xhEdtImgWidth" style="width:40px;" /> x <input type="text" id="xhEdtImgHeight" style="width:40px;" /></div><div>éæ¡å¤§å°ï¼<input type="text" id="xhEdtImgBorder" style="width:40px;" /></div><div>æ°´å¹³éè·ï¼<input type="text" id="xhEdtImgHspace" style="width:40px;" /> åç´éè·ï¼<input type="text" id="xhEdtImgVspace" style="width:40px;" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="ç¢ºå®" /></div>';
...
var htmlTable='<div>è¡æ¸åæ¸ï¼<input type="text" id="xhEdtTableRows" style="width:40px;" value="3" /> x <input type="text" id="xhEdtTableColumns" style="width:40px;" value="2" /></div><div>æ¨é¡å®åï¼<select id="xhEdtTableHeaders"><option selected="selected" value="">ç¡</option><option value="row">ç¬¬ä¸è¡</option><option value="col">ç¬¬ä¸å</option><option value="both">ç¬¬ä¸è¡åç¬¬ä¸å</option></select></div><div>å¯¬åº¦é«åº¦ï¼<input type="text" id="xhEdtTableWidth" style="width:40px;" value="200" /> x <input type="text" id="xhEdtTableHeight" style="width:40px;" value="" /></div><div>éæ¡å¤§å°ï¼<input type="text" id="xhEdtTableBorder" style="width:40px;" value="1" /></div><div>è¡¨æ ¼éè·ï¼<input type="text" id="xhEdtTableCellSpacing" style="width:40px;" value="1" /> è¡¨æ ¼å¡«åï¼<input type="text" id="xhEdtTableCellPadding" style="width:40px;" value="1" /></div><div>å°é½æ¹å¼ï¼<select id="xhEdtTableAlign"><option selected="selected" value="">é»èª</option><option value="left">å·¦å°é½</option><option value="center">å±ä¸­</option><option value="right">å³å°é½</option></select></div><div>è¡¨æ ¼æ¨é¡ï¼<input type="text" id="xhEdtTableCaption" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="ç¢ºå®" /></div>';
var htmlAbout='<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />çæ¬ï¼v0.9.9 build 20091123</p><p>xhEditoræ¯ä¸ååºæ¼jQueryéç¼çè·¨å¹³å°éæºè¿·ä½ XHTMLç·¨è¼¯å¨çµä»¶ã</p><p><a href="http://xheditor.com/" target="_blank">http://xheditor.com/</a></p></div>';
...
var arrTools={GStart:{},GEnd:{},Separator:{},Cut:{t:'åªå (Ctrl+X)'},Copy:{t:'è¤è£½ (Ctrl+C)'},Paste:{t:'ç²è²¼ (Ctrl+V)'},Pastetext:{t:'ç²è²¼ææ¬'},Blocktag:{t:'æ®µè½æ¨ç±¤'},Fontface:{t:'å­é«'},FontSize:{t:'å­è'},Bold:{t:'å ç² (Ctrl+B)',s:'Ctrl+B'},Italic:{t:'æé« (Ctrl+I)',s:'Ctrl+I'},Underline:{t:'ä¸åç· (Ctrl+U)',s:'Ctrl+U'},Strikethrough:{t:'ä¸­åç· (Ctrl+S)',s:'Ctrl+S'},FontColor:{t:'å­é«é¡è²'},BackColor:{t:'èæ¯é¡è²'},Removeformat:{t:'åªé¤æå­æ ¼å¼'},Align:{t:'å°é½'},List:{t:'åè¡¨'},Outdent:{t:'æ¸å°ç¸®é² (Shift+Tab)',s:'Shift+Tab'},Indent:{t:'å¢å ç¸®é² (Tab)',s:'Tab'},Link:{t:'è¶éæ¥'},Unlink:{t:'åæ¶è¶éæ¥'},Img:{t:'åç'},Flash:{t:'Flashåç«'},Media:{t:'è¦é »'},Emot:{t:'è¡¨æ'},Table:{t:'è¡¨æ ¼'},Source:{t:'æºä»£ç¢¼'},Preview:{t:'é è¦½'},Fullscreen:{t:'å¨å±ç·¨è¼¯ (Esc)',s:'Esc'},About:{t:'éæ¼ xhEditor'}};
var toolsThemes={
...
$.xheditor=function(textarea,options)
{
	...
	this.processHTML=function(sHtml,mode)
	{
		...
		{//write
			...
			sHtml = sHtml.replace(/<(\/?)del( [^>]+)?>/ig,'<$1strike$2>');//ç·¨è¼¯çæçµ±ä¸è½çºstrike
			if(isMozilla)
			{
			...
			if(!isSafari)
			{
				...
				sHtml = sHtml.replace(/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,style2font);//æè£¡å±¤
		...
		{//read
			...
			if(isSafari)
			{
				...
				sHtml = sHtml.replace(/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,replaceAppleSpan);//æè£¡å±¤
	...
	this.cleanWord=function(sHtml)
	{
		if(sHtml.match(/mso-|MsoNormal/i))
		{
			...
			//å±¬æ§æ¸ç
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+class="?mso[^ >]*([^>]*?)>/ig, "<$1$2>");//åªé¤ææmsoéé ­çæ¨£å¼
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+lang="?[^ \>]*([^>]*?)>/ig, "<$1$2>");//åªé¤langå±¬æ§
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+align="?left"?([^>]*?)>/ig, "<$1$2>");//åæ¶align=left
			//sHtml = sHtml.replace(/<(\w+[^>]*?) style="?[^">]+"?([^>]*?)>/ig, "<$1$2>");//åªé¤ææstyle
			...
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+style="?\s*([^">]+)\s*"?([^>]*?)>/ig, function(all,attr1,styles,attr2){return '<'+attr1+' style="'+styles.replace(/&quot;/ig,'')+'"'+attr2+'>';});//æ¸çstyleè£¡ç&quot;
		}
		return sHtml;
	}
	this.cleanHTML=function(sHtml)
	{
		...
		sHtml=sHtml.replace(/<\/(strong|b|u|strike|em|i)>((?:\s|<br\/?>|&nbsp;)*?)<\1>/ig,'$2');//é£çºç¸åæ¨ç±¤

		sHtml = sHtml.replace(/<(p|div)(?:\s+[^>]+)?>(((?!<\1(?: [^>]+)?>)[\s\S])+?)<\/\1>/ig,function(all,tag,content){//på§ç©ºç½é¡¯ç¤º
		...
		sHtml=sHtml.replace(/(\r?\n)+/g,"\r\n");//å¤è¡è®ä¸è¡
		sHtml=sHtml.replace(/\s+$/g,'');//çµå°¾æè¡
	...
	{//HTML Parser By John Resig (ejohn.org)
		...
		sHtml = sHtml.replace(/<(font)(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,font2style);//æè£¡å±¤
	...
	this.toggleSource=function(state)
	{
		...
		{//è½çºç·¨è¼¯æ¨¡å¼
			...
			{
				_doc.designMode = 'On';
				if(isMozilla)_this._exec("inserthtml","-");//ä¿®æ­£æºä»£ç¢¼åæåä¾ç¡æ³åªé¤æå­çåé¡
			}
	...
	{
		var s1='<span class="upload"><input type="text" style="visibility:hidden;" /><input type="button" value="ä¸å³" class="btn" />',jUpload;
		if(tourl.substr(0,1)=='!')//èªå®ç¾©ä¸å³ç®¡çé 
		{
			jUpload=$(s1+'</span>');
			jText.after(jUpload);
			$('.btn',jUpload).before(jText).click(function(){
				_this.showIframeModal(tourl.substr(1),'ä¸å³æä»¶',function(v){
					if(v.substr(0,1)=='!')
					{
		...
		{//ç³»çµ±é»èªajaxä¸å³
			...
			jFile.change(function(){
				...
				if(sFile!='')
				{
					...
					{
						var modal=_this.showModal('æä»¶ä¸å³','<div style="margin:22px 0;text-align:center;line-height:30px;">æä»¶ä¸å³ä¸­ï¼è«ç¨åâ¦â¦<br /><img src="'+skinPath+'img/loading.gif"></div>',320,150);
					...
					else alert('ä¸å³æä»¶æ´å±åå¿éçºï¼'+upext);
				}
			});
		}
	}
	this.ajaxUpload=function(fromfile,tourl,callback)
	{
		...
		jIO.load(function(){
			...
			else alert(tourl+' ä¸å³æ¥å£æé¯èª¤');
		});
	}
	...
	this.exec=function(cmd)
	{
		...
		switch(cmd)
		{
			case 'cut':
				...
				catch(ex){alert('æ¨ççè¦½å¨å®å¨è¨­ç½®ä¸åè¨±ä½¿ç¨åªåæä½ï¼è«ä½¿ç¨éµç¤å¿«æ·éµ(Ctrl + X)ä¾å®æ');};
				break;
			case 'copy':
				try{_doc.execCommand(cmd);if(!_doc.queryCommandSupported(cmd))throw 'Error';}
				catch(ex){alert('æ¨ççè¦½å¨å®å¨è¨­ç½®ä¸åè¨±ä½¿ç¨è¤è£½æä½ï¼è«ä½¿ç¨éµç¤å¿«æ·éµ(Ctrl + C)ä¾å®æ');}
				break;
			case 'paste':
				try{_doc.execCommand(cmd);if(!_doc.queryCommandSupported(cmd))throw 'Error';}
				catch(ex){alert('æ¨ççè¦½å¨å®å¨è¨­ç½®ä¸åè¨±ä½¿ç¨ç²è²¼æä½ï¼è«ä½¿ç¨éµç¤å¿«æ·éµ(Ctrl + V)ä¾å®æ');}
				break;
			case 'pastetext':
