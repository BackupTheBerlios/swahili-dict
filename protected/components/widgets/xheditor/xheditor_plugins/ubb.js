...
function ubb2html(sUBB)
{
	...
	sHtml=sHtml.replace(/\[code\]([\s\S]*?)\[\/code\]/ig,function(all,c){//codeç¹æ®å¤ç
...
function html2ubb(sHtml)
{
	...
	sUBB=sUBB.replace(/\[code\]([\s\S]*?)\[\/code\]/ig,function(all,c){//codeç¹æ®å¤ç
		cnum++;arrcode[cnum]=all;
		return "[\tubbcodeplace_"+cnum+"\t]";
	});
	...
	sUBB=sUBB.replace(/^(\r?\n)+/g,'');//æ¸é¤å¼å¤´æ¢è¡
	sUBB=sUBB.replace(/\s+$/g,'');//æ¸é¤ç»å°¾æ¢è¡
	
	for(i=1;i<=cnum;i++)sUBB=sUBB.replace("[\tubbcodeplace_"+i+"\t]", arrcode[i]);
	
	sUBB=sUBB.replace(/<[^<>]+?>/g,'');//å é¤ææHTMLæ ç­¾
