...
class awcforum_search{
   ...
   # die(">". $s);  
                 ...
                 $sql = "SELECT DISTINCT {$awc_tables['awc_f_threads']}.t_id, {$awc_tables['awc_f_threads']}.*, $awc_f_forums.f_name, $awc_f_forums.f_id, $awc_f_forums.f_name, $awc_f_cats.cat_name
                        FROM {$awc_tables['awc_f_threads']}
                        	INNER JOIN {$awc_tables['awc_f_posts']} awc_f_posts
...
global $awc_tables;
        ...
        if($morethenword) {
                  ...
                  foreach($sWords as $w){
                  	$w = $dbr->strencode(trim($w));
                    # die('>' . print_r($w));
                    $p .= " $what awc_f_posts.p_post LIKE '%" . $w . "%'";
                    $t .= " $what {$awc_tables['awc_f_threads']}.t_name LIKE '%" . $w . "%'";
                  }
        	...
        	$sWords = $dbr->strencode(trim($sWords));
            $p .= " awc_f_posts.p_post LIKE '%" . $sWords . "%'";
            $t .= " {$awc_tables['awc_f_threads']}.t_name LIKE '%" . $sWords . "%'";
